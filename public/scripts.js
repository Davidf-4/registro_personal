...
document.getElementById("login-form").addEventListener("submit", async function(event) {
    ...
    if (password === "admin123") {
        let response = await fetch("http://localhost:3000/registro")
...
document.getElementById("registration-form").addEventListener("submit", async function(event) {
    ...
    const estado = nucleo === 'aulaMovil' ? document.getElementById("estado").value : "";
    ...
    if (registros.some((registro, index) => registro.cedula === cedula && index !== editingIndex)) {
        alert("Ya existe un registro con esta cédula.");
        return;
    }

    const registro = { nombre, apellidos, cedula, telefono, direccion, nucleo, estado, area, tendencia };

    if (isEditing) {
        let response = await fetch('http://localhost:3000/registro/'+registros[editingIndex].id, {
            method: 'put',
            headers: {
...
function displayRegistros(registros) {
    ...
    registros.forEach((registro, index) => {
        ...
        newRow.innerHTML = `
            ...
            <td>${registro.nucleo}${registro.estado ? ` (${registro.estado})` : ""}</td>
            <td>${registro.area}</td>
        ...
        newRow.querySelector(".delete-btn").addEventListener("click", async function() {
            ...
            let response = await fetch('http://localhost:3000/registro/'+registro.id, {
                method: 'delete',
                headers: {
                    'Content-Type': 'application/json'
                }
...
document.getElementById("generate-pdf-search").addEventListener("click", function() {
...
function generatePDF(data) {
    ...
    logo1.onload = () => {
        logo2.onload = () => {
            ...
            const tableColumn = ["Nombre", "Apellidos", "Cédula", "Teléfono", "Dirección", "Núcleo", "Área", "Tendencia"];
            const tableRows = [];
